...
problem(no_power):-
    query('Does the computer do not turn on after pressing the button').
...
problem(usb_connect) :-
    query('Is the USB flash drive connected to the computer').
...
problem(disconnect) :-
    query('Is the Wi-Fi icon showing on the taskbar').
...
/*Внести изменения в данный Вам акинатор, добавив несколько объектов таким
образом, чтобы их можно было однозначно определить без введения дополнительных
вопросов, с введением дополнительных вопросов.*/

fault(power) :-
    problem(no_power).

fault(connect_internet) :-
    problem(disconnect).
